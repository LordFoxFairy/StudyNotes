...
在Go语言中，如果一个结构体包含map类型的字段，想要正确删除这个结构体，需要特殊处理map字段。

这是因为Go的垃圾回收机制不会自动回收结构体的map字段。所以如果直接删除结构体，map字段仍然存在，可能会造成内存泄漏。

正确删除包含map字段的结构体的方法是:遍历并清空该map字段，然后再删除整个结构体。示例：
...
在Go语言中，map的默认输出是无序的。但是我们可以通过两个方法实现map的有序输出：
1. 用slice保存map的键，然后根据slice进行排序。
...
2. 使用map的 variadic 的理念，将map存储到struct中。
...
func main() {
    ...
    sort.Slice(kvs, func(i,j int)bool{ // 结构体比较
...
通过这两个方法，就实现了map的有序输出。排序规则由``sort.Slice``指定。
...
在Go语言中， struct(结构体) 可以实现类似面向对象语言的继承关系。

具体来说，Go语言通过嵌套匿名结构体的方式实现继承：
...
```
这里，Dog 结构体嵌套了 Animal 结构体，Animal 作为 Dog 的匿名字段。

这就实现了继承关系：

- Dog 继承了 Animal 的所有字段(这里只有 Name 一个字段)
- 在 Dog类型的实例上可以访问 Animal 的字段
- Dog 添加了自己的新字段 LegNum

Dog 继承 Animal 的特点：

1. Dog 只继承了 Animal 的字段，不继承方法。
2. Dog 可以覆盖 Animal 的字段，通过自己的字段替代。 
3. Dog 可以添加新的字段。
4. 需要通过具体的结构体访问字段，没有 `class.field` 这种统一访问方式。

总的来说，Go语言通过嵌套结构体实现了继承关系，子结构体可以继承父结构体的字段。不同于面向对象语言的类继承，Go没有方法继承，也不能通过虚拟对象访问结构字段。但实现了类似的组合和继承效果。

### 结构体字段的可见性

Go语言中的结构体字段有三种可见性：

- 大写开头(Public)： 可在其他包外访问

- 小写开头(Private)： 仅在当前包内可访问

- 下划线开头(Underscore)： 像Private，但会被语法检查忽略(不推荐使用)
...
type Person struct {
    Name string // Public字段
    _age int    // Private字段，仅本包可访问 
}
```
...
func main() {
    p ：= person.Person{
        Name： "John"，
        _age： 30， // 编译报错，其他包无法访问Private字段
    }
    
    fmt.Println(p.Name) // OK，可以访问Public字段
}
```
编译上述`main.go`时，会报错`_age` not exported，无法访问包`person`中的Private字段。

所以可见性：

- Public字段可以在本包和其他包都访问

- Private字段仅可以在其所在包内访问

- Underscore字段与Private相同
...
func (p *Person) GetName() string { // Public方法
    return p.Name
}

func (p Person) print() {           // Private方法
   fmt.Println(p.Name) 
}
```

- 如果接受者是Public字段，则方法也是Public的。

- 如果接受者是Private字段，则方法也是Private的。

在其他包中：

- 可以调用Person的Public方法GetName()

- 但无法调用Person的Private方法 print()

### 结构体与 JSON 序列化

在Go语言中,可以很方便地将结构体与JSON进行序列化与反序列化。

#### 序列化结构体为JSON字符串

可以使用 `json.Marshal()` 来将结构体序列化为 JSON 字符串：
...
// {"Name":"John", "Age":30}
```
`json.Marshal()`函数的参数为任意类型，返回的是`[]byte`类型的JSON字符串。

#### JSON字符串反序列化为结构体
使用`json.Unmarshal()`可以将JSON字符串反序列化为对应的结构体：
```go
...
- 反序列化需要提供结构体指针，会直接修改该结构体。
- 字段名必须与JSON中的key对应，否则无法反序列化。
- JSON中缺少的字段在结构体中会使用默认值。
- 多出来的JSON字段会被忽略。

所以，通过 `json.Marshal()` 和 `json.Unmarshal()` ，可以实现结构体与JSON的完美转换，方便进行数据传输。
...
```go
fieldName string `json:"jsonName" xml:"xmlName"`
```

Tags可以在`json/xml`包内使用。例如：
...
- Name字段的Tag是`json:"name"` 
- Age字段的Tag 是`json:"age"` 

此时：

- 对Person进行JSON序列化时，Name字段会被编码为`"name":"xxx"` 
- Age字段会被编码为` "age":xxx` 
...
1. 可以自定义JSON/XML中的字段名
2. 让结构体字段与JSON/XML键对应起来，方便序列化/反序列化
3. 在编码/解码时忽略某些字段
...
func main() {
	...
	p := Person{Name: "John", Password: "123456"}

	jsonData, _ := json.Marshal(p)
	fmt.Println(string(jsonData))
}
// {"name": "John"}   (不包含Password字段)
```

所以，通过定义Tag，可以实现 JSON 文件名/XML元素名与结构体字段名不一致的情况，方便数据交换。这是Go语言结构体一个强大的特性。
...
```
if语句在Go语言中与其他语言类似，主要用于条件分支。主要的不同之处是：**Go没有三目运算符，使用条件表达式代替**。

### switch

Go语言中的switch case语句与其它编程语言类似。
...
它与if ... else if ... 对比:
- switch 的结构更清晰，当有很多分支时更易读
- case 可以是常量值、变量、类型、表达式

基本语法:
- variable 是要判断的变量或表达式
- case value 是与variable相等时的执行块
- default 是最后的Catch-all分支
...
Go语言的switch case还支持调用语句:

```go
switch os := os.Stdout; os {    
case nil:
    // ...
}
```

这里`os`在case中直接使用，不需要再定义。

**Go switch 的 case 也不需要 break 语句，编译器会自动终止**:

```go 
switch {
    case true:// 执行此分支后自动终止 
    case false:  
}
```
...
Go语言的select语句类似于switch，但它用于从多个输入通道(channel)中选择一个进行读取。
...
- 对应每个case语句：
  1. 读取语句`<-chan1`读取``chan1``通道。
  1. 写入语句`chan2<-` 向``chan2``通道写入。


- 默认default块，当没有case ready时执行。一个例子：
...
这里从 c1 或 c2 通道中读取字符串，打印到终端。

#### 规则

Go语言中的select操作符用于从多个通信选择一个进行运行。它具有如下规则：
1. **从它的多个case语句中，选择一个准备就绪的case。如果有多个case都准备就绪，则选择其中一个随机。**
2. 如果没有case准备就绪，而有default句柄，则运行default句柄。
3. 如果没有case和default句柄准备就绪，select会阻塞，直到某个case就绪。
4. 每个case最多选择一个通道操作。
5. case语句可以有赋值语句，用于向变量中存放从通道接收到的数据。
6. case语句可以是发送操作或接收操作。
7. default 通常用于超时或处理无关通道的情况。
...
```
- initialization ：循环初始化。`for`循环中的初始化语句，只会执行一次。
- condition ：循环条件。 每次迭代开始前执行。
- post ：每个迭代结束后执行。 

在`for`循环内，可以修改这3个表达式的值，控制循环的执行。
...
- continue 会退出当前迭代，继续下一次迭代。

#### for + range

用于遍历数组、切片、集合、 channel、字符串等：
...
当定义一个函数时，需要指定函数名、参数列表和返回值类型。Go语言的函数定义格式如下：
...
其中，参数列表和返回值类型都可以省略。如果没有返回值，则可以省略返回值类型；如果没有参数，则可以省略参数列表。
...
在调用函数时，需要传递对应的参数，并根据需要接收返回值。例如：
...
需要值得注意的是，在Go语言中，**函数的左大括号必须和函数名在同一行**，而右大括号则可以单独占一行或者和函数体在同一行。

### 函数特点

- 支持多返回值：Go语言函数可以返回多个值，这在其他编程语言中比较少见。
- 支持匿名函数和闭包：Go语言支持匿名函数和闭包，可以方便地实现一些高级编程技巧。
- 函数是一等公民：在Go语言中，函数是一等公民，可以像变量一样被传递、赋值、作为参数或返回值等。
- 可变参数：Go语言支持可变参数，在参数列表中使用…表示。这样可以方便地处理不定数量的参数。

### 参数

#### 参数类型

在Go语言中，函数的参数可以是任何类型，包括基本类型、结构体、数组、切片、映射等。同时，Go语言还支持可变参数，在参数列表中使用``…``表示。
...
func printPersonInfo(name string, age int, address string) {
...
在Go语言中，函数参数传递有两种方式：值传递和引用传递。默认情况下是值传递，即将实参的值复制一份给形参。如果需要引用传递，则需要使用指针类型。
...
在Go语言中，函数可以返回多个值。如果只需要返回一个值，则可以省略括号；如果需要返回多个值，则需要使用括号将它们括起来。
...
在Go语言中，函数支持可变参数，在参数列表中使用``…``表示。这样可以方便地处理不定数量的参数。
...
在Go语言中，函数也支持闭包。闭包是指一个函数和它所引用的外部变量组合成的一个整体。通过闭包可以实现一些高级编程技巧。
...
在Go语言中，函数可以返回单个值。如果需要返回一个值，则可以在函数定义时指定该类型的返回值。
...
在Go语言中，函数可以返回多个值。如果需要返回多个值，则需要使用括号将它们括起来。
...
在Go语言中，还可以为函数的返回值命名。这样，在函数体内就不需要再次声明变量了。
...
在Go语言中，如果不需要某个返回值，则可以使用空白符_来忽略它。
...
在Go语言中，匿名函数是一种没有函数名的函数，它可以直接在代码中定义和使用。匿名函数通常用于需要在某个地方定义一个简单的逻辑块，并且不需要给它起一个独立的函数名的场景。

以下是一个示例代码，演示了如何定义和使用匿名函数：
...
在这个示例中，首先定义了一个匿名函数并立即调用它，计算了1和2的和，并将结果赋值给result变量。然后，将另一个匿名函数赋值给multiply变量，并调用它计算了3和4的乘积，并将结果赋值给product变量。

需要注意的是，在定义匿名函数时，使用了``func``关键字，并在后面紧跟着参数列表和返回类型（如果有）。然后，在大括号内编写具体的逻辑代码。最后，在整个表达式末尾加上小括号并传入参数列表来调用该匿名函数。

### 闭包

闭包是一种特殊的函数，它**可以访问其外部作用域中的变量，并将其保存在函数内部**。换句话说，闭包是一个函数和其相关的引用环境的组合体。
...
在这个示例中，我们定义了一个counter函数，它返回一个内部匿名函数。内部匿名函数可以访问并修改外部作用域中的count变量。

在main函数中，我们调用counter函数并将返回的内部匿名函数赋值给变量c。通过调用c，我们可以使用闭包来递增和获取count变量的值。

第一次调用c时，count的初始值为0，然后count被递增为1，并返回1作为结果。**第二次调用c时，由于count变量仍然存在于闭包中，并且闭包保持了对count变量的引用**，所以count的值仍然是上一次调用后的结果（即1），然后count被递增为2，并返回2作为结果。

通过使用闭包，我们可以创建具有状态和行为的功能块，并且可以在需要时保持其状态。这对于一些需要记住某些信息或上下文状态的场景非常有用。

### 递归

递归是一种在函数内部调用自身的技术。它是解决问题的一种常见方法，特别是对于可以分解为较小子问题的问题。

下面是一个示例，展示了如何使用递归计算一个数的阶乘：
...
func main() {
    result := factorial(5)
    fmt.Println(result) // 输出120
}
```

在这个示例中，我们定义了一个名为 factorial 的函数，它接受一个整数参数n，并返回n的阶乘。在函数内部，我们首先检查n是否等于0，如果是，则返回1作为基本情况。否则，我们通过调用factorial函数来计算 n-1 的阶乘，并将结果与 n 相乘。

递归可以非常强大和灵活，但需要小心使用。确保设置好适当的终止条件以避免无限循环，并确保每次递归调用都能使问题规模减小。
...
在Go语言中，延迟调用（defer）是一种机制，允许我们在函数执行完毕前安排一些操作。这些操作会被推迟执行，直到包含它们的函数返回时才会被执行。

##### 特性和作用

延迟调用的特性和作用如下：

- **defer的定义**：defer是一个关键字，用于定义一个延迟调用。它允许我们将一个函数或方法的调用推迟到包含该defer语句的函数返回之前执行。
- **延迟执行**：defer语句中的函数或方法调用不会立即执行，而是在包含函数执行完毕前被推迟执行。
- **多个defer语句**：一个函数可以包含多个defer语句，它们按照逆序执行，即最后一个defer语句将最先执行。
- **清理和收尾操作**：defer通常用于执行一些清理和收尾操作，比如关闭文件、释放资源、解锁互斥锁等。通过defer，可以确保这些操作在函数返回之前被执行，无论函数是正常返回还是发生了异常。
- **简化错误处理**：defer还可以用于简化错误处理。通过在函数开头设置defer语句，即使在函数执行过程中发生错误，也能够确保相关的清理操作得到执行。

##### defer语句的语法形式和用法

defer语句的语法形式为：`defer functionCall(arguments)`

- `functionCall`：要被推迟执行的函数或方法调用。
- `arguments`：可选的参数，用于传递给被调用函数或方法的参数。
...
在上面的示例中，调用`fmt.Println("World")`的defer语句被推迟执行，直到包含它的函数`foo()`返回前才会执行。因此，函数`foo()`首先会打印"Hello"，然后在返回前会执行defer语句，打印"World"。

通过使用defer，我们可以确保在函数执行完毕之前执行一些必要的清理或收尾操作，提高代码的可读性和可维护性。

#### defer的用途

##### 在函数执行完毕前进行清理和收尾操作的实际用途

在函数执行过程中，可能需要进行一些清理或收尾操作，比如关闭文件、释放资源、解锁互斥锁等。使用defer语句可以确保这些操作在函数返回前被执行，无论是正常返回还是发生了异常。

例如，在打开文件进行读取操作后，我们希望在函数返回前关闭该文件，以避免资源泄漏。使用defer语句可以很方便地实现这一点，无论函数在何处返回，文件都会被正确关闭。
...
在上述示例中，通过defer语句将文件关闭操作推迟到函数返回前执行，即使函数发生了错误并提前返回，文件也会被关闭。

##### 如何使用defer来确保资源的释放和关闭

defer语句常用于确保资源的释放和关闭，比如关闭文件、关闭数据库连接等。通过将资源释放操作推迟到函数返回前执行，我们可以有效地管理和避免资源泄漏。

例如，在使用数据库时，我们通常会打开数据库连接，并在使用完毕后关闭它。使用defer语句可以确保数据库连接的关闭操作在函数返回前执行。
...
在上述示例中，通过defer语句将数据库连接的关闭操作推迟到函数返回前执行，无论函数是否发生了错误，数据库连接都会被正确关闭。

#### defer与闭包

##### 延迟调用和闭包的相互影响

当在闭包中使用defer语句时，需要注意闭包中捕获的变量的值在执行defer时会被确定。也就是说，defer语句中使用的变量值是在定义时就已经确定的，而不是在实际执行时确定的。
...
在上述示例中，我们创建了一个循环并在每次迭代中使用了一个匿名函数作为defer语句。该匿名函数打印变量`i`的值。

然而，如果我们运行上述代码，会发现打印的结果是三个3，而不是我们可能期望的0、1、2。这是因为在每次迭代中，匿名函数都捕获了同一个变量`i`的地址，而不是每次迭代都创建一个新的变量。

在函数返回前执行defer语句时，闭包中的`i`已经被修改为3，因此每个defer语句都打印的是相同的值。

##### 闭包中的defer语句的执行顺序和注意事项

当在闭包中使用defer语句时，需要注意闭包中捕获的变量的值在执行defer时会被确定。也就是说，defer语句中使用的变量值是在实际执行时确定的，而不是在定义时确定的。
...
在上述示例中，我们创建了一个循环，并在每次迭代中使用一个匿名函数。该匿名函数中包含了defer语句，用于打印变量`i`的值。

然而，如果我们运行上述代码，会发现打印的结果是三个3，而不是我们可能期望的0、1、2。这是因为在每次迭代中，匿名函数都共享同一个变量`i`，并且在函数返回前执行defer语句时，闭包中的`i`已经被修改为3，因此每个defer语句都打印的是相同的值。

为了解决这个问题，我们可以在循环中创建一个新的变量，将迭代变量的值复制给新变量，并在闭包中使用新变量。这样每个闭包都会捕获不同的变量，避免了上述的问题。

```go
func main() {
    for i := 0; i < 3; i++ {
        j := i // 创建一个新变量，并将i的值复制给它
        func() {
            defer fmt.Println(j)
        }()
    }
...
通过上述修改，每个闭包都捕获了不同的变量`j`，而不是共享同一个变量`i`。这样，打印的结果将会是0、1、2。

#### defer与文件操作（defer f.Close）

##### 如何使用defer确保文件的正确关闭

使用defer语句可以方便地确保在文件操作完成后正确关闭文件，以避免资源泄漏。在Go语言中，通过在打开文件后使用defer语句来推迟文件的关闭操作，确保在函数返回前执行。

以下是一个示例，展示了如何使用defer确保文件的正确关闭：
...
在上述示例中，我们打开了一个文件并使用defer语句将文件的关闭操作推迟到函数返回前执行。无论在读取文件内容时发生错误还是函数正常返回，都会确保文件被正确关闭。

##### 通过示例展示defer在文件操作中的应用

以下是一个完整的示例，展示了如何使用defer在文件操作中打开、读取和关闭文件：
...
在上述示例中，我们使用`os.Open`函数打开名为 "data.txt" 的文件，并使用defer语句将文件的关闭操作推迟到`main`函数返回前执行。然后，我们使用`file.Read`函数读取文件的内容，并将其打印出来。

通过使用defer语句，我们可以确保在任何情况下（无论是读取文件时发生错误，还是函数正常返回），文件都会被正确关闭。

#### defer的陷阱

##### defer与return语句的相互作用和注意事项

在使用defer和return语句时，需要特别注意它们的相互作用。当函数中同时存在defer语句和有名返回值时，需要注意defer语句的执行时机。

在函数执行到return语句时，先执行返回值的赋值操作，然后再执行defer语句，最后函数返回。
...
在上述示例中，我们在函数`process`中使用了一个defer语句来对返回值`result`进行增加操作。然后，我们使用return语句返回了初始值0。

然而，如果我们运行上述代码，打印的结果将是1，而不是期望的0。这是因为在有名返回值的情况下，return语句会先更新返回值，然后再执行defer语句。

##### 处理nil函数的defer语句

在使用defer语句时，如果defer的函数参数是一个nil函数，会导致程序在执行到该defer语句时产生panic。
...
在上述示例中， test 的函数一直运行至结束，然后 defer 函数会被执行且会因为值为 nil 而产生 panic 异常。然而值得注意的是，run() 的声明是没有问题，因为在test函数运行完成后它才会被调用。
...
异常处理在Go语言中主要依靠panic和recover机制来实现。当遇到严重错误或异常情况时，可以使用panic抛出一个异常，然后通过defer和recover来捕获和处理这个异常。

#### 异常处理的使用场景

在以下情况下，可以使用panic抛出异常，并通过defer和recover来捕获和处理异常：

1. 程序遇到无法恢复的错误，例如内存溢出、文件不存在等。此时可以使用panic抛出异常，并在高层级的函数中通过recover进行捕获和处理。
...
func main() {
    ...
    // 调用readData函数，捕获可能发生的异常
    readData()
}
```

2. 在并发程序中，当一个goroutine发生严重错误时，可以使用panic抛出异常，并通过recover在主goroutine中捕获和处理异常。
...
func main() {
    go func() {
        ...
        // 在goroutine中调用doSomething函数，捕获可能发生的异常
        doSomething()
    }()
...
注意，在使用panic抛出异常时，程序会立即停止当前函数的执行，并开始执行defer语句。defer语句中的recover用于捕获异常，然后可以对异常进行处理或恢复程序执行。

但是需要注意的是，panic和recover应该作为异常处理的最后一道防线，而不是常规的错误处理机制。在大部分情况下，应该使用返回错误值的方式来处理错误。

#### recover 和 panic 的注意事项
...
在Go语言中，使用 panic 关键字可以引发一个异常。当遇到无法恢复的错误或异常情况时，可以使用 panic 抛出异常，并终止当前函数的执行。异常会沿着调用栈向上冒泡，直到被捕获或导致程序终止。

以下是使用panic进行异常处理的一些要点：

1. panic 可以在任何地方引发异常，它会立即停止当前函数的执行，并开始执行defer语句。
2. 在引发 panic 后，程序会终止当前 goroutine 的执行，但其他 goroutine 仍然可以继续执行。
3. 可以使用 defer 和 recover 来捕获并处理 panic 引发的异常。defer语句用于设置清理操作，而 recover 函数用于捕获异常并进行处理。
...
在上述示例中，当满足某个条件时，调用 doSomething 函数会引发panic异常。然后，通过 defer 语句和 recover 函数来捕获并处理该异常。

使用 panic 的异常处理机制适用于那些无法恢复的错误或异常，它提供了一种快速终止程序执行并进行清理操作的方式。

##### 使用error处理错误

在Go语言中，使用error接口类型来表示错误，并通过返回错误值的方式进行错误处理。通过判断错误对象实例来确定具体错误类型，可以使用 errors.New 和 fmt.Errorf 函数来创建实现error接口的错误对象。

以下是使用error进行错误处理的一些要点：

1. 错误处理使用返回值的方式，而不是抛出异常。
2. 错误通常作为函数的最后一个返回值，并且是error类型。
3. 错误值可以通过与nil进行比较来判断是否发生了错误。
4. 错误值可以包含详细的错误信息，以便在处理错误时提供更多的上下文信息。
...
在上述示例中，doSomething 函数返回一个错误值，如果满足某个条件，则返回一个包含错误信息的错误对象。然后，通过与nil进行比较来检查是否发生了错误，并在发生错误时进行相应的处理。

使用 error 进行错误处理是一种更加结构化和可控的方式，可以根据具体的错误类型采取不同的处理逻辑。

综上所述，使用 panic 和 error 都是处理错误和异常的方式，但它们在用途和机制上有所不同。panic 适用于无法恢复的错误或异常情况，而 error 适用于常规的错误处理和错误类型判断。

## 方法

### 方法定义

在Go语言中，方法是与特定类型相关联的函数。方法定义在结构体类型上。方法的定义格式如下：

```go
func (receiverName ReceiverType) MethodName(args) (returns) {
    // method body
}
```

其中，receiverName 是接收器的名称，ReceiverType 是接收器的类型，MethodName 是方法名称，args 是方法参数列表，returns 是返回值列表。

接收器可以是值类型或指针类型，但是方法的定义必须与接收器类型一致。如果接收器是值类型，则方法中对接收器的修改不会影响原始值；如果接收器是指针类型，则方法中对接收器的修改会影响原始值。

> 注意，方法和普通函数的主要区别在于它们的接收器。
>
> - 方法是与类型相关联的函数，它们具有一个接收器，可以是该类型的值或指针。由于方法与类型关联，所以它们可以访问和修改该类型的内部数据，而且方法调用时会自动传递接收器作为第一个参数。
> - 普通函数是直接定义在包级别的，它们没有接收器，只有输入参数和返回值，且不能访问该类型的任何内部数据。

### 匿名字段

Go 允许在结构体类型中使用匿名字段，这意味着该类型可以包含另一个结构体类型的字段，而无需指定该类型的名称。这也可以用于在匿名字段上定义方法。以下是一个示例：
...
func (s SomeOtherStruct) SomeMethod() {
	fmt.Println("Hello from SomeMethod!")
}

func main() {
	myInstance := MyStruct{SomeOtherStruct{"hi"}, 42}
	myInstance.SomeMethod() // 输出 "Hello from SomeMethod!"
}
```

在上面的示例中，`MyStruct`包含一个匿名字段`SomeOtherStruct`，该字段具有`anotherField`和`SomeMethod`方法。因此，`MyStruct`的实例也可以调用`SomeMethod`方法。

### 方法集

Go语言中的方法集定义了类型可以调用哪些方法。方法集由类型的接收器类型和方法名称组成。方法集包括以下几种类型的方法：

- 值接收器方法：接收器类型为值类型的方法；
- 指针接收器方法：接收器类型为指针类型的方法；
- 值和指针接收器方法：接收器类型既可以是值类型也可以是指针类型的方法。

对于值类型的方法，只有该类型的值和该类型的指针可以调用该方法。对于指针类型的方法，该类型的指针和该类型的值都可以调用该方法。对于既可以使用值类型也可以使用指针类型的方法，该类型的值和该类型的指针都可以调用该方法。

### 表达式

在Go语言中，方法也可以是一个表达式，可以将其赋值给一个变量或作为函数参数传递。
...
func main() {
    r := Rect{3, 4}
    areaFunc := r.Area // 将方法赋值给变量
    fmt.Println(areaFunc()) // Output: 12
}
```


在上面的例子中，将 Rect 的方法 Area 赋值给变量 areaFunc，然后调用该变量即可得到计算面积的结果。

### 自定义error

在Go语言中，错误处理是通过返回一个错误对象来完成的。标准库中的错误类型是error接口，它只有一个方法Error()，返回一个字符串描述错误的详细信息。如果函数返回的错误对象为nil，则表示没有错误。

可以通过实现error接口来定义自己的错误类型，例如：
...
在上面的示例中，MyError 实现了error接口的Error()方法，该方法返回message字段的值作为错误描述信息。

定义了自己的错误类型之后，可以在代码中使用它们来表示特定的错误情况。例如，以下是一个使用自定义错误类型的示例函数：
...
在上面的示例中，doSomething()函数返回一个*MyError类型的错误对象，该对象在发生错误时被创建并返回。可以在调用doSomething()函数时检查返回的错误对象是否为nil，如果为nil则表示没有错误，否则就是发生了错误。

另外，Go语言还提供了fmt.Errorf()函数，可以用来创建一个新的错误对象。例如：

```go
func doSomething() error {
    if someCondition {
        return fmt.Errorf("Something went wrong: %v", someCondition)
    }
    return nil
}
```

在上面的示例中，fmt.Errorf() 函数创建了一个带有错误描述信息的新错误对象，并返回该对象。%v格式化动词用于将someCondition的值插入到错误描述信息中。

## 面向对象

面向对象编程（Object-Oriented Programming，简称 OOP）是一种编程范式，它将数据和操作数据的方法组织在一起，形成对象。在面向对象编程中，程序的主要构成是对象，而不是过程。对象是由数据和方法组成的实体，它们可以相互通信和协作。

在面向对象编程中，有两个重要的概念：匿名字段和接口。

### 匿名字段

匿名字段是指在结构体或类中，定义一个字段时不指定字段的名称，只指定字段的数据类型。通过匿名字段，可以在一个结构体或类中直接访问另一个结构体或类的字段和方法。

在 Go 语言中，可以通过结构体嵌套来实现匿名字段。例如：
...
在上面的例子中，`Employee` 结构体中包含了 `Person` 结构体作为匿名字段。这意味着 `Employee` 中包含了 `Person` 结构体的所有字段和方法，可以直接通过 `Employee` 对象访问 `Person` 的字段和方法：
...
fmt.Println(emp.name) // 访问 Person 结构体的 name 字段
fmt.Println(emp.age)  // 访问 Person 结构体的 age 字段
```

### 接口

接口是一种抽象类型，它定义了一组方法的集合。接口定义了方法的签名，但不包含方法的实现。在 Go 语言中，接口是一种隐式实现的，只要一个类型实现了接口中定义的所有方法，就被认为实现了该接口。
...
其中，`MyInterface` 是接口的名称，接口中定义了一组方法。每个方法都由方法名、参数列表和返回类型组成。

一个类型要实现一个接口，只需要实现接口中定义的所有方法即可。例如：
...
在上面的例子中，定义了一个 `Shape` 接口，该接口包含了两个方法 `Area()` 和 `Perimeter()`。然后，定义了一个 `Circle` 结构体，并为其实现了 `Area()` 和 `Perimeter()` 方法。因此，`Circle` 类型实现了 `Shape` 接口。

可以通过类型断言来判断一个对象是否实现了某个接口：
...
func main() {
    ...
    if ok { 
        fmt.Println("shape is a Circle", circle)
    } else {
        fmt.Println("shape is not a Circle", circle)
    }
}
```
...
![img](./assets/01-Go基础篇/m_c420fc1f2fa7ad164993b9118e851625_r.png)
...
![img](./assets/01-Go基础篇/m_180a6ea2e7e7df801de5160a892c9de2_r.png)
...
![img](./assets/01-Go基础篇/m_7a3fd62eebab50a962857bf9a421495a_r.png)
